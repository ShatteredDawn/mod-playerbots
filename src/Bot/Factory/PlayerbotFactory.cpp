...
void PlayerbotFactory::Refresh()
{
    ...
    if (bot->GetLevel() >= sPlayerbotAIConfig.minEnchantingBotLevel && !PlayerbotAIConfig::instance().disableRandomBotPeriodicRandomization)
        ApplyEnchantAndGemsNew();
...
void PlayerbotFactory::InitEquipment(bool incremental, bool second_chance)
{
    ...
    if (isPvp && level >= 50 && !racialHasCcBreak)
    {
        for (uint32 itemId : ccBreakTrinketCache)
        {
            ...
            if (proto->Quality > itemQuality) continue;
    ...
    if (second_chance)
    {
        for (int32 slot : initSlotsOrder)
        {
            ...
            Item* const equipped = bot->EquipNewItem(dest, bestItemForSlot, true);
