...
bool IccFestergutGroupPositionAction::PositionNonTankMembers()
{
    ...
    float xOffset = (float(col) - 2) * spreadDistance;                // Center around tank position
    float yOffset = minBossDistance + (float(row) * spreadDistance);  // Each row further back
...
Position IccFestergutSporeAction::CalculateSpreadPosition()
{
    ...
    float angle = float((bot->GetGUID().GetCounter() % 16) * (M_PI / 8));
