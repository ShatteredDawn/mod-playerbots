...
bool IccBqlGroupPositionAction::HandleShadowsMovement()
{
    ...
    int shadowCount = 0;
    for (uint32_t i = 0; i < npcs.size() && shadowCount < MAX_SHADOW_NPCS; i++)
    {
    ...
    for (int curveIdx = 0; curveIdx < MAX_CURVES; curveIdx++)
    {
        ...
        score += float(curveIdx) * OUTER_CURVE_PREFERENCE;
...
bool IccBqlGroupPositionAction::HandleGroupPosition(Unit* boss, Aura* frenzyAura, Aura* shadowAura)
{
    ...
    if (bossAirborne)
    {
        ...
        uint32_t myIndex = std::numeric_limits<uint32_t>::max();
        ...
        if (myIndex == std::numeric_limits<uint32_t>::max())
        {
            return false;
        }
        ...
        if (myAssignedSlot < 0)
        {
            ...
            for (uint32_t attempt = 0; attempt < totalSlots; attempt++)
            {
                int s;
                if (isHunter)
...
bool IccBqlPactOfDarkfallenAction::Execute(Event /*event*/)
{
    ...
    return MoveToTargetPosition(targetPos, int(playersWithAura.size() + 1));  // +1 to include the bot itself
}
...
bool IccBqlPactOfDarkfallenAction::CalculateCenterPosition(Position& targetPos, const std::vector<Player*>& playersWithAura)
{
    ...
    float totalPlayers = float(playersWithAura.size() + 1);  // +1 for the bot itself
