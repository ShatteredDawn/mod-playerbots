...
static Position ComputeGroupCentroid(Player* bot)
{
    ...
    return Position(sumX / float(count), sumY / float(count), bot->GetPositionZ());
}
...
bool IccLichKingShadowTrapAction::Execute(Event /*event*/)
{
    ...
    for (int i = 1; i <= TEST_POSITIONS + 1; ++i)
    {
        ...
        float const testAngle = (i <= TEST_POSITIONS)
            ? currentAngle - ANGLE_STEP * float(i)
...
bool IccLichKingWinterAction::TryMoveToPosition(float targetX, float targetY, float targetZ, bool forced)
{
    ...
    for (int i = 1; i <= STEPS; ++i)
    {
        for (int sign : {1, -1})
        {
            float const arc = float(M_PI) / STEPS * float(i) * float(sign);
            float const cosA = std::cos(arc);
            float const sinA = std::sin(arc);
...
bool IccLichKingSpiritBombAction::Execute(Event)
{
    ...
    for (float const radius : SearchDistances)
    {
        ...
        {
            float const angle = float(i) * 2.0f * float(M_PI) / ANGLE_COUNT;
            ...
            float const score = minHDist - float(nearbyCount) * DENSITY_PENALTY + distBonus;
...
bool IccLichKingAddsAction::HandleRagingSpiritFlanking(Unit* boss, bool hasPlague, Difficulty diff)
{
    ...
    if (isRanged)
    {
        ...
        for (int i = 0; i < ANGLE_STEPS; ++i)
        {
            ...
            float const angle = baseAngle + float(sign * offset) * (2.0f * float(M_PI) / ANGLE_STEPS);
    ...
    for (float const radius : MeleeRings)
    {
        for (int i = 0; i < ANGLE_STEPS; ++i)
        {
            float const angle = (float(i) * 2.0f * float(M_PI)) / ANGLE_STEPS;
...
bool IccLichKingAddsAction::HandleDefileMechanics(Unit* boss, Difficulty diff)
{
    ...
    if (spiritCount > 0)
    {
        float const cx = spiritSumX / float(spiritCount);
        float const cy = spiritSumY / float(spiritCount);
    ...
    for (int offset = 0; offset <= 8 && !found; ++offset)
    {
        for (int dir : {-1, 1})
        {
            ...
            float const angle = baseAngle + float(dir * offset) * float(M_PI) / 8.0f;
...
bool IccLichKingAddsAction::HandleValkyrAssignment(std::vector<Unit*> const& grabbingValkyrs)
{
    ...
    int64_t const myIndex = std::distance(assistMembers.begin(), it);
    auto const groupSizes = CalculateBalancedGroupSizes(assistMembers.size(), valid.size());
    size_t const valkyrIndex = GetAssignedValkyrIndex(size_t(myIndex), groupSizes);
