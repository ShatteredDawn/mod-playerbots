        ...
        if (boss && boss->IsAlive() && boss->GetVictim() == bot && !boss->HealthBelowPct(35))
        {
            ...
            if (!nearPuddles.empty())
            {
                ...
                for (int i = 0; i < kiteAngleSteps; ++i)
                {
                    ...
                    int sign = (i % 2 == 0) ? 1 : -1;
                    int step = (i + 1) / 2;
                    float angle = seedAngle + float(sign * step) *
...
Position IccPutricideGrowingOozePuddleAction::CalculateSafeMovePosition(Unit* closestPuddle)
{
    ...
    {
        float angle = (2.0f * static_cast<float>(M_PI) * float(i)) / numAnglesToTest;
...
bool IccPutricideGasCloudAction::HandleGaseousBloatMovement(Unit* gasCloud)
{
    ...
    {
        float cA = (2.0f * static_cast<float>(M_PI) * float(i)) / cornerCheckDirs;
    ...
    if (blockedAtStart >= 3)
    {
        ...
        for (int i = 0; i < 32; ++i)
        {
            float escAngle = (2.0f * static_cast<float>(M_PI) * float(i)) / 32;
            ...
            {
                float cA = (2.0f * static_cast<float>(M_PI) * float(j)) / checkDirs;
            ...
            for (int k = 0; k < 8; ++k)
            {
                float cA = (2.0f * static_cast<float>(M_PI) * float(k)) / 8;
    ...
    {
        float angle = (2.0f * static_cast<float>(M_PI) * float(i)) / numAngles;
        float rotatedDx = dx * std::cos(angle) - dy * std::sin(angle);
        float rotatedDy = dx * std::sin(angle) + dy * std::cos(angle);
        ...
        for (float testDist = movementIncrement; testDist <= maxTestDist; testDist += movementIncrement)
        {
                ...
                bot->IsWithinLOS(testX, testY, testZ))
            {
                ...
                for (int j = 0; j < checkDirs; ++j)
                {
                    float checkAngle = (2.0f * static_cast<float>(M_PI) * float(j)) / checkDirs;
    ...
    if (!boss->HealthBelowPct(35))
    {
        ...
        if (botInDanger)
        {
            ...
            for (float r : radii)
            {
                ...
                {
                    float a = (2.0f * float(M_PI) * float(i)) / angleSteps;
