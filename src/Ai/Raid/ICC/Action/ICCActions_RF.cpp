...
bool IccRotfaceTankPositionAction::HandleBigOozeKiting(Unit* bigOoze)
{
    ...
    {
        float const angleOffset = float((i % 2 == 0 ? 1 : -1) * (M_PI / 16.0f) * (float(i) / 2.0f));
...
bool IccRotfaceGroupPositionAction::MoveAwayFromPuddle(Unit* boss, Unit* puddle, float)
{
    ...
    {
        float const testAngle = float(angle + M_PI + (i * M_PI / 4));
        ...
        float const newCenterDistance = float(std::sqrt(std::pow(moveX - ICC_ROTFACE_CENTER_POSITION.GetPositionX(), 2) +
...
bool IccRotfaceGroupPositionAction::PositionHeroicGrid(Unit* boss)
{
    ...
    if (hasHomeSpot)
    {
        ...
        if (displacedRank < static_cast<int32>(tempSpots.size()))
            return moveTowardSpot(rangedSpots[tempSpots[uint32_t(displacedRank)]]);
    }
...
bool IccRotfaceMoveAwayFromExplosionAction::Execute(Event /*event*/)
{
    ...
    if (currentSlotIt != sExplosionSlotMemory.end())
    {
        ...
        auto [x, y] = slotPos(uint32_t(mySlot));
        if (otherCount[size_t(mySlot)] < 2 && isSlotSafe(x, y))
    ...
    {
        auto [x, y] = slotPos(uint32_t(finalSlotIt->second));
...
bool IccRotfaceAvoidVileGasAction::Execute(Event /*event*/)
{
    ...
    if (!_hasSafeSpot)
    {
        ...
        for (int32 i = 0; i < sweepSteps; ++i)
        {
            ...
            float const angle = baseAngle + float(sign * magnitude) * stepAngle;
