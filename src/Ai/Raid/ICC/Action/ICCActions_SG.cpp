...
bool IccSindragosaFrostBeaconAction::TryDropTombFlares(Unit const* boss)
{
    ...
    if (myIt == markers.end())
        return false;
    size_t const mySlot = size_t(std::distance(markers.begin(), myIt));
    ...
    uint32 const spellId = uint32_t(flare->GetTemplate()->Spells[0].SpellId);
...
bool IccSindragosaFrostBeaconAction::HandleBeaconedPlayer(const Unit* boss)
{
    ...
    size_t const myIndex = size_t(std::distance(beaconedPlayers.begin(), it));
...
bool IccSindragosaFrostBombAction::Execute(Event /*event*/)
{
    ...
    Position const& myZone = *tombZones[uint32_t(zoneIdx)];
    ...
    for (ObjectGuid const& guid : currentGuids)
    {
        ...
        if (s_groupAssignments.find(key) == s_groupAssignments.end())
        {
            ...
            for (int g = 1; g < groupCount; ++g)
                if (counts[uint32_t(g)] < counts[uint32_t(minGroup)])
                    minGroup = g;
...
void IccSindragosaFrostBombAction::PinGroupToCurrentZone()
{
    ...
    if (myTomb)
    {
        ...
        {
            uint32_t const zoneIdx = (groupCount == 2) ? (g == 0 ? 0 : 2) : uint32_t(g);
...
std::vector<Unit*> IccSindragosaFrostBombAction::SelectTombs(std::vector<Unit*> const& tombs, int groupIndex, int groupCount) const
{
    ...
    for (Unit* tomb : tombs)
    {
        ...
        for (int z = 1; z < 3; ++z)
        {
            ...
            if (groupCount == 2 && z == 1)
                continue;
            float d = tomb->GetExactDist2d(*tombZones[uint32_t(z)]);
            if (d < closestDist)
            {
                closestDist = d;
                closestZone = z;
