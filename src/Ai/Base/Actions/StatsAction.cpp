...
void StatsAction::ListGold(std::ostringstream& out) { out << ChatHelper::formatMoney(bot->GetMoney()); }
...
void StatsAction::ListRepairCost(std::ostringstream& out)
{
    ...
    out << "|c" << color << (uint32)ceil(repairPercent) << "% (" << ChatHelper::formatMoney(totalCost) << ")|cffffffff Dur";
}
