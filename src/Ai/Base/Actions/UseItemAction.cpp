...
bool UseItemAction::Execute(Event event)
{
    ...
    GuidVector gos = ChatHelper::parseGameobjects(name);
...
bool UseItemAction::UseGameObject(ObjectGuid guid)
{
    ...
    out << "Using " << ChatHelper::FormatGameobject(go);
...
bool UseItemAction::UseItem(Item* item, ObjectGuid goGuid, Item* itemTarget, Unit* unitTarget)
{
    ...
    std::ostringstream out;
    out << "Using " << ChatHelper::FormatItem(item->GetTemplate());
    ...
    if (goGuid)
    {
        ...
        out << " on " << ChatHelper::FormatGameobject(go);
        targetSelected = true;
    }
    ...
    if (itemTarget)
    {
        ...
        else
        {
            ...
            out << " on " << ChatHelper::FormatItem(itemTarget->GetTemplate());
            targetSelected = true;
        }
    }
    ...
    if (uint32 questid = item->GetTemplate()->StartQuest)
    {
        if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(questid))
        {
            ...
            std::ostringstream out;
            out << "Got quest " << ChatHelper::FormatQuest(qInfo);
            botAI->TellMasterNoFacing(out.str());
            return true;
        }
    }
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; i++)
    {
        ...
        if (spellInfo->Targets & TARGET_FLAG_ITEM)
        {
            ...
            else
            {
                ...
                targetSelected = true;
                out << " on " << ChatHelper::FormatItem(itemForSpell->GetTemplate());
            }
...
{
    std::ostringstream out;
    out << action << " " << ChatHelper::FormatItem(item->GetTemplate());
...
bool UseItemAction::SocketItem(Item* item, Item* gem, bool replace)
{
    ...
    {
        std::ostringstream out;
        out << "Socketing " << ChatHelper::FormatItem(item->GetTemplate());
        out << " with " << ChatHelper::FormatItem(gem->GetTemplate());
