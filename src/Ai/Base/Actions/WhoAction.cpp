...
std::string const WhoAction::QueryTrade(std::string const text)
{
    ...
    for (Item* sell : items)
    {
        ...
        out << "Selling " << ChatHelper::FormatItem(sell->GetTemplate(), sell->GetCount()) << " for "
            << ChatHelper::formatMoney(sellPrice);
        return out.str();
    }
...
{
    std::ostringstream out;
    uint32 skill = ChatHelper::ParseSkill(text);
    if (!skill || !botAI->HasSkill((SkillType)skill))
        return "";

    std::string const skillName = ChatHelper::FormatSkill(skill);
...
std::string const WhoAction::QuerySpec(std::string)
{
    ...
    out << "|h|cffffffff" << ChatHelper::FormatRace(bot->getRace()) << " [" << (bot->getGender() == GENDER_MALE ? "M" : "F")
        << "] " << ChatHelper::FormatClass(bot, spec);
