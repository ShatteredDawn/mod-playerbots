...
bool SetCraftAction::Execute(Event event)
{
    ...
    ItemIds itemIds = ChatHelper::parseItems(link);
    if (itemIds.empty())
    {
...
void SetCraftAction::TellCraft()
{
    ...
    out << "I will craft " << ChatHelper::FormatItem(proto) << " using reagents: ";
    ...
    for (std::map<uint32, uint32>::iterator i = data.required.begin(); i != data.required.end(); ++i)
    {
        ...
        if (ItemTemplate const* reagent = sObjectMgr->GetItemTemplate(item))
        {
            ...
            out << ChatHelper::FormatItem(reagent, required);
    ...
    out << " (craft fee: " << ChatHelper::formatMoney(GetCraftFee(data)) << ")";
