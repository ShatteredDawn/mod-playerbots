...
void TradeStatusAction::BeginTrade()
{
    ...
    if (sRandomPlayerbotMgr.IsRandomBot(bot))
    {
        ...
        if (discount)
        {
            ...
            out << "Discount up to: " << ChatHelper::formatMoney(discount);
...
bool TradeStatusAction::CheckTrade()
{
    ...
    if (!botAI->HasActivePlayerMaster() && GET_PLAYERBOT_AI(bot->GetTrader()))
    {
        ...
        if (isGettingItem)
        {
            if (bot->GetGroup() && bot->GetGroup()->IsMember(bot->GetTrader()->GetGUID()) &&
                ...
                botAI->TellMasterNoFacing("Thank you " + ChatHelper::FormatWorldobject(bot->GetTrader()));
            else
                bot->Say("Thank you " + ChatHelper::FormatWorldobject(bot->GetTrader()),
    ...
    for (uint32 slot = 0; slot < TRADE_SLOT_TRADED_COUNT; ++slot)
    {
        ...
        {
            std::ostringstream out;
            out << ChatHelper::FormatItem(item->GetTemplate()) << " - This is not for sale";
            botAI->TellMaster(out);
        ...
        if (item)
        {
            ...
            {
                std::ostringstream out;
                out << ChatHelper::FormatItem(item->GetTemplate()) << " - I don't need this";
                botAI->TellMaster(out);
                botAI->PlaySound(TEXT_EMOTE_NO);
                return false;
            }
    ...
    std::ostringstream out;
    out << "I want " << ChatHelper::formatMoney(-(delta + discount)) << " for this";
    botAI->TellMaster(out);
    botAI->PlaySound(TEXT_EMOTE_NO);
    return false;
}
