...
std::vector<Item*> InventoryAction::parseItems(std::string const text, IterateItemsMask mask)
{
    ...
    uint32 quality = ChatHelper::parseItemQuality(text);
    ...
    if (ChatHelper::parseItemClass(text, &itemClass, &itemSubClass))
    {
    ...
    uint32 fromSlot = ChatHelper::parseSlot(text);
    ...
    ids = ChatHelper::parseItems(text);
    for (ItemIds::iterator i = ids.begin(); i != ids.end(); i++)
    {
        FindItemByIdVisitor visitor(*i);
        IterateItems(&visitor, mask);
