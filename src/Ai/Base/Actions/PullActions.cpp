...
bool PullStartAction::Execute(Event event)
{
    ...
    const NextAction::Factory preActionFactory = strategy->GetPullPreActionFactory();

    if (preActionFactory != nullptr && !botAI->DoSpecificAction(preActionFactory, event, true))
        return false;
...
std::vector<NextAction> PullAction::getPrerequisites()
{
    ...
    return IsWithinPullRange(bot, target, strategy) ? std::vector<NextAction>{}
                                                     : std::vector<NextAction>{ CreateNextAction<ReachPullAction>(ACTION_MOVE) };
}
...
bool PullAction::Execute(Event event)
{
    ...
    const NextAction::Factory actionFactory = strategy->GetPullActionFactory();

    if (!botAI->DoSpecificAction(actionFactory, event, true))
        return false;
