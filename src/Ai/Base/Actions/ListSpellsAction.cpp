...
std::vector<std::pair<uint32, std::string>> ListSpellsAction::GetSpellList(std::string filter)
{
    ...
    if (!ss.empty())
    {
        skill = ChatHelper::ParseSkill(ss[0]);
        if (skill != SKILL_NONE)
        {
    ...
    if (filter.find('+') != std::string::npos)
    {
        ...
        if (skill == SKILL_NONE)
        {
            ...
            {
                uint32 parsedSkill = ChatHelper::ParseSkill(skillTokens[0]);
    ...
    uint32 slot = ChatHelper::parseSlot(filter);
        ...
        for (uint32 x = 0; x < MAX_SPELL_REAGENTS; ++x)
        {
            ...
            if (itemid)
            {
                if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemid))
                {
                    ...
                    materials << ChatHelper::FormatItem(proto, reagentsRequired);
