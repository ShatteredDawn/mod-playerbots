...
{
    std::vector<Item*> petitions = AI_VALUE2(std::vector<Item*>, "inventory items", ChatHelper::FormatQItem(5863));
...
{
    return !bot->GetGuildId() && AI_VALUE2(uint32, "item count", ChatHelper::FormatQItem(5863)) &&
...
bool PetitionTurnInAction::isUseful()
{
    ...
    return inCity && !bot->GetGuildId() && AI_VALUE2(uint32, "item count", ChatHelper::FormatQItem(5863)) &&
           AI_VALUE(uint8, "petition signs") >= sWorld->getIntConfig(CONFIG_MIN_PETITION_SIGNS) &&
...
bool BuyTabardAction::Execute(Event)
{
    ...
    if (canBuy && AI_VALUE2(uint32, "item count", ChatHelper::FormatQItem(5976)))
        return true;
...
bool BuyTabardAction::isUseful()
{
    ...
    return inCity && bot->GetGuildId() && !AI_VALUE2(uint32, "item count", ChatHelper::FormatQItem(5976)) &&
