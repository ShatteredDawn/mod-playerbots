...
{
    return Event("rpg action", ChatHelper::FormatWorldobject(this->rpg->guidP().GetWorldObject()));
}
...
bool RpgTradeUsefulAction::Execute(Event)
{
    ...
    param << ChatHelper::FormatWorldobject(player);
    param << " ";
    param << ChatHelper::FormatItem(item->GetTemplate());
        ...
        if (bot->GetTradeData() && bot->GetTradeData()->HasItem(item->GetGUID()))
        {
            if (bot->GetGroup() && bot->GetGroup()->IsMember(guidP) && botAI->HasRealPlayerMaster())
                botAI->TellMasterNoFacing(
                    "You can use this " + ChatHelper::FormatItem(item->GetTemplate()) + " better than me, " +
                    guidP.GetPlayer()->GetName() /*ChatHelper::FormatWorldobject(guidP.GetPlayer())*/ + ".");
            else
                bot->Say("You can use this " + ChatHelper::FormatItem(item->GetTemplate()) + " better than me, " +
                             player->GetName() /*ChatHelper::FormatWorldobject(player)*/ + ".",
                         (bot->GetTeamId() == TEAM_ALLIANCE ? LANG_COMMON : LANG_ORCISH));

            if (!urand(0, 4) || items.size() < 2)
            {
                // bot->Say("End trade with" + ChatHelper::FormatWorldobject(player), (bot->GetTeamId() == TEAM_ALLIANCE ?
            ...
            bot->Say("Start trade with" + ChatHelper::FormatWorldobject(player),
                     (bot->GetTeamId() == TEAM_ALLIANCE ? LANG_COMMON : LANG_ORCISH));
...
bool RpgDuelAction::Execute(Event)
{
    ...
    return botAI->DoSpecificAction(CreateNextAction<CastCustomSpellAction>(1.0f).factory, Event("rpg action", ChatHelper::FormatWorldobject(player) + " 7266"),
                                   true);
}
