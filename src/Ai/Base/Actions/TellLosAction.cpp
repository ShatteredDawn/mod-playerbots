...
void TellLosAction::ListGameObjects(std::string const title, GuidVector gos)
{
    ...
    for (ObjectGuid const guid : gos)
    {
        if (GameObject* go = botAI->GetGameObject(guid))
            botAI->TellMaster(ChatHelper::FormatGameobject(go));
    }
}
...
bool TellCalculateItemAction::Execute(Event event)
{
    ...
    ItemWithRandomProperty item = ChatHelper::parseItemWithRandomProperty(text);
    ...
    out << "Calculated score of " << ChatHelper::FormatItem(proto) << " : " << score;
