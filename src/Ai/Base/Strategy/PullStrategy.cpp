...
private:
    static ActionNode* pull_start(PlayerbotAI* /*botAI*/)
    {
        return new ActionNode(
            {},
            {},
            { CreateNextAction<PullAction>(ACTION_NORMAL) }
        );
    }
};

PullStrategy::PullStrategy(PlayerbotAI* botAI, std::string const action, const NextAction::Factory actionFactory, std::string const preAction, const NextAction::Factory preActionFactory)
    : Strategy(botAI), action(action), actionFactory(actionFactory), preAction(preAction), preActionFactory(preActionFactory)
{
...
std::vector<NextAction> PullStrategy::getDefaultActions()
{
    ...
    return {
        CreateNextAction<PullAction>(105.0f),
    };
}
...
void PullStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    triggers.push_back(new TriggerNode(
        "pull start",
        {
            CreateNextAction<PullStartAction>(106.0f),
            CreateNextAction<PullAction>(ACTION_MOVE)
        }
    ));
    ...
    triggers.push_back(new TriggerNode(
        "pull end",
        {
            CreateNextAction<PullEndAction>(107.0f)
        }
    ));
}
...
void PullBackStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        ...
        {
            CreateNextAction<ReturnToPullPositionAction>(ACTION_MOVE + 5.0f)
        }
    ));
}
